/*
 * Improved summarizer that shows per-child "...(+N more)" lines in large
 * folders, giving a clearer view of distribution across sub-directories.
 */

export function summarizeFilePaths(
  rawPaths: string[],
  {
    smallGroupLimit = 15,
    exampleCount = 3,
  ...
  for (const folder of Object.keys(groups).sort()) {
    ...
    if (files.length <= smallGroupLimit) {
      lines.push(`${folder}/`);
      for (const rel of files.sort()) lines.push(`  ${rel.slice(folder.length + 1)}`);
      continue;
    }
    ...
    const omittedChildCount = Object.keys(childGroups).length - childNames.length;
