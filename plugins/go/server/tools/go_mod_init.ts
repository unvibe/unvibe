...
export const config = make({
  name: 'go_mod_init',
  description:
    'Initialize a Go module in the project directory with the given module name.',
  usage:
    '**go_mod_init**: Initialize a Go module.\n\nParams:\n- moduleName (string, required): The module name for go.mod',
  parameters: {
...
export const createTool = ({ project }: { project: Project }) => ({
  ...
  fn: async (args: unknown) => {
    ...
    try {
      const output = await runShellCommand(`go mod init ${moduleName}`, {
        cwd,
      });
