  ...
  return (
    <div className='bg-background-1 rounded-3xl shadow-xl p-10 max-w-md w-full flex flex-col items-center animate-fade-in border border-border'>
      ...
      <div className='flex items-center gap-3 mb-2'>
        ...
        <Badge color='primary' className='ml-2'>{profile.role}</Badge>
      </div>
      ...
      <div className='mb-6 w-full'>
        <label className='block text-foreground-2 text-xs mb-1 font-semibold'>Bio</label>
        {editMode ? (
          <textarea
            value={edit.bio}
      ...
      <div className='flex gap-4 mt-2'>
        {!editMode ? (
          <Button variant='primary' onClick={() => { setEdit(profile); setEditMode(true); }}>
            Edit Profile
          </Button>
        ) : (
          <>
